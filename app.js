...
function markDirty(id,showToast=true){
 ...
 // 防抖写入 KV，刷新后仍能看到上次选项（即使尚未点应用）
 if(settingsSaveTimer)clearTimeout(settingsSaveTimer);
 settingsSaveTimer=setTimeout(()=>{persistSettingsToKv().catch(()=>{})},800);
}
...
function loadSource(text,label,opts={}){
 ...
 if(!opts.silentToast)toast(`已加载 ${label}，无需重新注册。`,"success","原生配置加载成功");
 // 默认持久化到 KV（从 KV 恢复时 skipPersist，避免重复写）
 if(opts.persist!==false && !opts.fromKv){
   persistConfigToKv(label).catch(()=>{});
 }
}

async function persistConfigToKv(label=""){
  ...
  try{
    const res=await fetch("/api/config/save",{
      ...
      body:JSON.stringify({config:source,label:label||"usque-config"})
    });
    const data=await res.json().catch(()=>({}));
    if(!res.ok)throw new Error(data.message||`HTTP ${res.status}`);
    setStatus("原生 config.json 已永久保存到 KV，刷新后自动恢复。","ok");
    toast("注册信息已写入 KV，刷新页面也会保留。","success","已永久保存");
    updateConfigKvStatus(true,data.meta);
    return true;
  }catch(e){
    toast("保存到 KV 失败："+e.message+"（请确认已绑定 USQUE_KV）","warning","KV 保存失败");
    updateConfigKvStatus(false,null,e.message);
    return false;
  }
}

function updateConfigKvStatus(has,meta,err){
  const el=$("configKvStatus");
  if(!el)return;
  if(err){
    el.className="config-kv-status error";
    el.textContent="KV 状态：保存/读取失败 — "+err;
    return;
  }
  if(has){
    el.className="config-kv-status ok";
    const t=meta?.updated_at?`，更新于 ${meta.updated_at}`:"";
    el.textContent=`KV 状态：已保存原生 config.json${t}（刷新后自动恢复）`;
  }else{
    el.className="config-kv-status";
    el.textContent="KV 状态：尚未保存注册信息。注册或导入后会自动写入。";
  }
}

async function restoreConfigFromKv(){
  try{
    const res=await fetch("/api/config/load",{cache:"no-store",credentials:"same-origin"});
    if(!res.ok){
      const t=await res.text();
      updateConfigKvStatus(false,null,t||`HTTP ${res.status}`);
      return false;
    }
    const data=await res.json();
    if(!data.kv_bound){
      updateConfigKvStatus(false,null,"未绑定 USQUE_KV");
      return false;
    }
    if(!data.has_config||!data.config){
      updateConfigKvStatus(false,null);
      return false;
    }
    loadSource(JSON.stringify(data.config), data.meta?.label || "KV 已保存的 config.json", {
      fromKv:true,
      persist:false,
      silentToast:true
    });
    toast("已从 KV 自动恢复上次注册的 config.json。","success","自动恢复");
    setStatus("已从 KV 恢复原生配置，可直接应用设置 / 推送订阅。","ok");
    updateConfigKvStatus(true,data.meta);
    return true;
  }catch(e){
    updateConfigKvStatus(false,null,e.message);
...
async function clearConfigFromKv(){
  if(!confirm("确定清除 KV 中保存的原生 config.json？刷新后将不再自动恢复。"))return;
  try{
    const res=await fetch("/api/config/clear",{
      method:"POST",
      credentials:"same-origin",
      headers:{"Content-Type":"application/json"}
    });
    const data=await res.json().catch(()=>({}));
    if(!res.ok)throw new Error(data.message||`HTTP ${res.status}`);
    toast("KV 中的注册信息已清除。","success","已清除");
    updateConfigKvStatus(false,null);
  }catch(e){
    toast("清除失败："+e.message,"error","清除失败");
  }
}
...
async function register(){
 ...
 try{
  ...
  setStatus("注册成功！已先生成原生 config.json。正在写入 KV…","ok");
  ...
  // 永久保存到 KV
  persistConfigToKv("网页注册生成的 usque-config.json").catch(()=>{});
 }catch(e){
...
const SETTINGS_FIELD_IDS = [
  "freeEgressEnabled","freeUseWarp","freeScope","freeProtocolMode",
  "freeUS","freeJP","freeSG","freeHK","freeTW","freeKR",
  "customIpRoutingEnabled","customIpRoutingRules",
  "endpointPreset","customEndpoint","portPreset","customPort",
  "sniPreset","customSni","dnsPreset","customDns",
  "innerIpMode","customInner4","customInner6",
  "networkMode","stackMode","ccMode","outerCc","bbrProfile",
  "mtuPreset","customMtu","handshakeTimeout","ruleMode",
  "healthUrl","healthInterval","healthTolerance","aiHealthMode",
  "chatgptRouteMode","otherAiRouteMode","chatgptDirectFallback",
  "egressCountry1","egressCountry2","egressCountry3",
  "controllerUrl","controllerSecret","localProxyUrl",
  "egressChatgptFirst","egressApplyAll","enableControllerApi",
  "nodeTag","profileName","nodeCountMode","customNodeCount",
  "nodeBuildMode","extraEndpoints","nodeNaming",
  "udpForward","remoteDns","autoSelect","showIcons","adBlock",
  "includeTestEndpoints","dedupeNodes","h2ExtendedPool",
  "regDeviceName","autoDownloadConfig"
];
...
function collectUiSettings(){
  ...
  for(const id of SETTINGS_FIELD_IDS){
    const el=$(id);
    if(!el)continue;
    if(el.type==="checkbox") out[id]={t:"c",v:!!el.checked};
    else out[id]={t:"v",v:el.value};
  }
  // 记住当前输出格式
  out._currentFormat={t:"v",v:currentFormat||"native"};
  out._activePreset={t:"v",v:document.querySelector(".preset.active")?.id||"presetRecommended"};
  return out;
}

function applyUiSettings(map){
  ...
  syncConditional();
  updateEstimate();
  updateFreeEgressHint();
  updateCustomIpRoutingHint();
  try{updateChatgptRouteHint()}catch(_){}
  settingsDirty=false;
  setSettingsState("clean","已恢复上次保存的界面设置");
  return applied>0;
}

async function persistSettingsToKv(){
  try{
    ...
    toast("界面设置保存到 KV 失败："+e.message,"warning","设置未持久化");
    return false;
  }
}

async function restoreSettingsFromKv(){
  try{
    const res=await fetch("/api/settings/load",{cache:"no-store",credentials:"same-origin"});
    if(!res.ok)return false;
    const data=await res.json();
    if(!data.has_settings||!data.settings)return false;
    applyUiSettings(data.settings);
    toast("已恢复上次的界面设置（高级选项等）。","success","设置已恢复");
    return true;
  }catch(e){
...
$("resetSettings").addEventListener("click",()=>{
  resetRecommended();settingsDirty=true;setSettingsState("dirty","已恢复推荐设置，点击“应用设置并重新生成”后生效");
  toast("所有高级设置已恢复为推荐值。","success","恢复成功");
  if(settingsSaveTimer)clearTimeout(settingsSaveTimer);
  settingsSaveTimer=setTimeout(()=>{persistSettingsToKv().catch(()=>{})},400);
});
$("applySettings").addEventListener("click",async ()=>{
 ...
 try{
  ...
  // 仅保存界面设置到 KV，不自动推送订阅（订阅请手动点「推送当前 Clash 到订阅」）
  await persistSettingsToKv();
...
/* ---------------- 在线订阅（KV） ---------------- */
let subInfoCache=null;

function setSubStatus(html,kind=""){
  const el=$("subStatusBox"); if(!el)return;
  el.className="online-sub-status"+(kind?" "+kind:"");
  el.innerHTML=html;
}

async function fetchSubInfo(){
  const res=await fetch("/api/sub/info",{cache:"no-store",credentials:"same-origin"});
  if(!res.ok){
    const t=await res.text();
...
async function refreshSubInfo(showToast=false){
  const input=$("subUrlInput");
  try{
    const info=await fetchSubInfo();
    subInfoCache=info;
    if(input)input.value=info.url||"";
    const parts=[];
    if(!info.kv_bound)parts.push("⚠ 未绑定 KV（变量名须为 <code>USQUE_KV</code>）");
    else parts.push("✅ KV 已绑定");
    if(!info.uuid)parts.push("⚠ 未配置 <code>SUB_UUID</code>");
    else parts.push("✅ SUB_UUID 已配置");
    if(info.has_content){
      parts.push(`✅ 已有订阅内容（${formatSubBytes(info.bytes)}，来源 ${info.source||"?"}）`);
      if(info.meta?.updated_at)parts.push(`更新时间：${info.meta.updated_at}`);
      if(info.meta?.note)parts.push(`备注：${info.meta.note}`);
    }else{
      parts.push("尚无内容：请先生成 Clash，再点击「推送当前 Clash 到订阅」");
    }
    const kind=(!info.kv_bound||!info.uuid)?"warn":(info.has_content?"ok":"warn");
    setSubStatus(parts.join("<br>"),kind);
    if(showToast)toast("订阅状态已刷新。","success","订阅状态");
    return info;
  }catch(e){
    if(input)input.value="";
    setSubStatus("无法读取订阅状态："+e.message+"（若未登录或会话过期，请重新打开本页登录）","error");
    if(showToast)toast("刷新订阅状态失败："+e.message,"error","订阅状态");
    return null;
  }
}

async function pushClashToSubscription(opts={}){
  const silent=!!opts.silent;
  const fromApply=!!opts.fromApply;
  if(!source){
    if(!silent)toast("请先注册或导入 config.json。","error","无法推送");
    return false;
  }
  let yaml="";
  try{
    // 始终用当前设置重新生成 Clash，避免推送旧内容
    yaml=buildClash();
    if(currentFormat!=="clash"){
      try{setFormat("clash")}catch(_){}
    }else{
      $("mainOutput").value=yaml;
    }
  }catch(e){
    if(!silent)toast("生成 Clash 失败："+e.message,"error","推送失败");
    setSubStatus("❌ 生成 Clash 失败："+e.message,"error");
    return false;
  }
  if(!yaml||!String(yaml).trim()){
    if(!silent)toast("Clash 内容为空，无法推送。","error","推送失败");
    return false;
  }

  const btn=$("pushSubClash");
  const old=btn?btn.textContent:"";
  if(btn){btn.disabled=true;btn.textContent="推送中…"}
  setSubStatus("正在写入 Cloudflare KV…","");

  try{
    const noteParts=[
      $("profileName")?.value||"MASQUE-Pro",
      `${connectionPairs().length}nodes`,
      $("freeEgressEnabled")?.checked?`free:${selectedFreeLabels().join("+")||"off"}`:"warp-only",
      fromApply?"auto-apply":"manual"
    ];
    const res=await fetch("/api/sub/save",{
      method:"POST",
      credentials:"same-origin",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({
    ...
    if(data.url&&$("subUrlInput"))$("subUrlInput").value=data.url;
    setSubStatus(
      `✅ 已推送到在线订阅<br>链接：<code>${data.url||"(请刷新状态)"}</code><br>大小：${formatSubBytes(data.bytes)}`+
      (data.meta?.updated_at?`<br>时间：${data.meta.updated_at}`:""),
      "ok"
    );
    toast(
      fromApply
        ? "设置已应用，Clash 已自动推送到在线订阅。"
        : "Clash 配置已写入 KV，订阅链接已是最新内容。",
      "success",
      "推送成功"
    );
    subInfoCache=null;
    ...
    setSubStatus("❌ 推送失败："+e.message+"<br>请确认已绑定 KV（变量名 USQUE_KV）并设置了 SUB_UUID。","error");
    toast("推送失败："+e.message,"error","推送失败");
    return false;
  }finally{
    if(btn){btn.disabled=false;btn.textContent=old||"⬆ 推送当前 Clash 到订阅"}
  }
}

async function clearSubscription(){
  if(!confirm("确定清除 KV 中的订阅内容？订阅链接将暂时没有可用 YAML。"))return;
  try{
    const res=await fetch("/api/sub/clear",{
      method:"POST",
      credentials:"same-origin",
      headers:{"Content-Type":"application/json"}
    });
    const data=await res.json().catch(()=>({}));
    if(!res.ok)throw new Error(data.message||`HTTP ${res.status}`);
    toast("KV 订阅内容已清除。","success","已清除");
    await refreshSubInfo(false);
  }catch(e){
    toast("清除失败："+e.message,"error","清除失败");
  }
}

$("copySubUrl")?.addEventListener("click",()=>{
  const v=$("subUrlInput")?.value||"";
  if(!v){toast("暂无订阅链接，请先配置 SUB_UUID 并刷新状态。","warning","无链接");return}
  copyText(v,$("copySubUrl"));
});
$("refreshSubInfo")?.addEventListener("click",()=>refreshSubInfo(true));
$("pushSubClash")?.addEventListener("click",()=>pushClashToSubscription());
$("clearSubContent")?.addEventListener("click",()=>clearSubscription());

$("clearConfigKv")?.addEventListener("click",()=>clearConfigFromKv());
$("saveConfigKv")?.addEventListener("click",()=>{
  if(!source){toast("当前没有已加载的 config.json。","error","无法保存");return}
  persistConfigToKv($("loadedFile")?.textContent?.replace(/^✓\s*/,"")||"manual-save");
});

refreshSubInfo(false);

// 启动：默认推荐值 → 恢复界面设置 → 恢复注册 config
(async()=>{
  resetRecommended();
  syncConditional();updateEstimate();updateFreeEgressHint();updateCustomIpRoutingHint();
  const restoredSettings=await restoreSettingsFromKv();
  if(!restoredSettings){
    setSettingsState("clean","当前为推荐设置（尚未保存过自定义选项）");
  }
  const ok=await restoreConfigFromKv();
  if(!ok){
    setStatus("等待开始：请注册 Usque，或导入已有 config.json（将自动保存到 KV）。","");
  }
})();
})();